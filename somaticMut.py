...
def create_name2(row):
    return f"{row['Gene']}_{row['Name']}"
...
print(data_matrix)
oncoFrame = pd.DataFrame(columns=['GT', 'AD', 'AF', 'DP'])
split_data = data_matrix['OtherInfo10'].str.split(':', expand=True)
split_data = split_data.filter([0, 1, 2, 3])
split_data.columns = ['GT', 'AD', 'AF', 'DP']
split_data = split_data[split_data['GT'].str.match(r'^(0/1|0\|1|1/1|1\|1|1/0|1\|0)$')]
split_data['AF'] = pd.to_numeric(split_data['AF'])
...
filtered_df2['Name2'] = filtered_df2.apply(create_name2, axis=1)
print("FILTERED 2")
print(filtered_df2)
...
secondFrame_filtered = secondFrame[nan_counts >= 4]
print(secondFrame_filtered)
print("HIT")
print(secondFrame)
